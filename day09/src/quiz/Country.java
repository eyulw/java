...
public class Country {
	public static void main(String[] args) {
		...
		while(true) {
			//한국의 수도는? (입력)
			int idx = (int)(Math.random()*nationList.size());
			String country =nationList.get(idx).getCountry();
			System.out.println(country+"의 수도는? ");
			String capital = scanner.next();
			if(capital.equals("exit")) break;
			//정답이 맞으면 딩동댕 틀리면 땡
			if(capital.equals(nationList.get(idx).getCapital())){
